...
max_fb_acc_ = 9 
Kpos    = np.array([-2,-2,-4])
Kvel_ = np.array([-2,-2,-3])

   
def afb_term (pos_error, vel_error ):
    a_fb = Kpos * pos_error + Kvel_ * vel_error

    if (LA.norm(a_fb) > max_fb_acc_):
        a_fb = (max_fb_acc_ / LA.norm(a_fb)) * a_fb
    return a_fb

def acc2quaternion(vector_acc, yaw):

    proj_xb_des = np.array([np.cos(yaw), np.sin(yaw), 0.0])

    zb_des = vector_acc / LA.norm(vector_acc)
    yb_des = np.cross(zb_des,proj_xb_des) / LA.norm(np.cross(zb_des,proj_xb_des))
    xb_des = np.cross(yb_des,zb_des) / LA.norm(np.cross(yb_des,zb_des))

    phi = np.arctan2(yb_des[2],zb_des[2])
    theta = -np.arcsin(xb_des[2])
    return phi,theta
...
class SupportFilesDrone:
    ...
    def pos_controller_SMC_U1_PD (self,X_ref,X_dot_ref,X_dot_dot_ref,Y_ref,Y_dot_ref,Y_dot_dot_ref,Z_ref,Z_dot_ref,Z_dot_dot_ref,Psi_ref,states):
        ...
        vel_x= float(pos_vel_fixed[0]) 
        vel_y= float(pos_vel_fixed[1])
        vel_z= float(pos_vel_fixed[2])
        #Target _ desire value 
        Ref_pos = np.array([X_ref, Y_ref ,Z_ref])
        Ref_velo = np.array([X_dot_ref, Y_dot_ref ,Z_dot_ref])
        Ref_acc = np.array([X_dot_dot_ref, Y_dot_dot_ref ,Z_dot_dot_ref])
        # current velocity 
        curr_pos = np.array([x , y ,z])
        curr_velo = np.array([vel_x, vel_y,vel_z])
        
        pos_error = curr_pos - Ref_pos
        vel_error = curr_velo - Ref_velo

        g = np.array([0, 0 ,g])
        #Position Controller
        a_fb = afb_term(pos_error, vel_error)

        #Reference acceleration
        a_des = a_fb + Ref_acc  + g 

        mavYaw_ = psi
        U1 = np.dot(a_des,(0,0,1))
        # Compute the errors
        ex=X_ref-x
        ex_dot=X_dot_ref-vel_x
        ey=Y_ref-y
        ey_dot=Y_dot_ref-vel_y
    ...
    def pos_controller_PID (self,X_ref,X_dot_ref,X_dot_dot_ref,Y_ref,Y_dot_ref,Y_dot_dot_ref,Z_ref,Z_dot_ref,Z_dot_dot_ref,Psi_ref,states):
        ...
        v_x= float(pos_vel_fixed[0]) 
        v_y= float(pos_vel_fixed[1])
        v_z= float(pos_vel_fixed[2])
        #Target _ desire value 
        Ref_pos = np.array([X_ref, Y_ref ,Z_ref])
        Ref_velo = np.array([X_dot_ref, Y_dot_ref ,Z_dot_ref])
        Ref_acc = np.array([X_dot_dot_ref, Y_dot_dot_ref ,Z_dot_dot_ref])
        # current velocity 
        curr_pos = np.array([x , y ,z])
        curr_velo = np.array([v_x, v_y,v_z])
        ...
        mavYaw_ = psi
        #Reference attitude
        Phi_ref , Theta_ref= acc2quaternion(a_des, mavYaw_)
        U1 = np.dot(a_des,(0,0,1))
        return Phi_ref, Theta_ref, U1
