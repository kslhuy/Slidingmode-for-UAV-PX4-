...
%        c = 3;
  % Calcule differentiator    
       ...
       [x_0, x_1,x_2] = differentiator1(x1,xp_x,A.Ts,u_obs_x,lamda1y,lamda2y,lamda3y);
       [y_0, y_1,y_2] = differentiator1(y1,xp_y,A.Ts,u_obs_y,lamda1y,lamda2y,lamda3y);
...
%         
  ...
  SW2_x = SW2_x+ k3x*sat(s_x)*A.Ts;
%   
%   SW3_x = SW3_x+ s_x*A.Ts;
  
  SW1_y = SW1_y+ sat(y1 - y1_hat)*A.Ts;
  
  SW2_y = SW2_y+ k3y*sat(s_y)*A.Ts;
%   
%   SW3_y = SW3_y+ s_y*A.Ts;

  %%SWSMC

%   A.Ux = (  -cx*x2_hat + cx*x1_dot_des - lamda3x*SW1_x - lamda2x*(abs(x1-x1_hat)^1/3)*sat(x1 - x1_hat) + x2_dot_des - k1x*(abs(s_x)^1/2)*sat(s_x) - k3x*SW2_x);
%   A.Uy = (  -cy*y2_hat + cy*y1_dot_des - lamda3y*SW1_y - lamda2y*(abs(y1-y1_hat)^1/3)*sat(y1 - y1_hat) + y2_dot_des - k1y*(abs(s_y)^1/2)*sat(s_y) - k3y*SW2_y);
% %   
    A.Ux = (  -cx*x2_hat + cx*x1_dot_des  -dis_x_filtre - lamda2x*(abs(x1-x1_hat)^1/3)*sat(x1 - x1_hat) + x2_dot_des - k1x*(abs(s_x)^1/2)*sat(s_x) - SW2_x);
    A.Uy = (  -cy*y2_hat + cy*y1_dot_des -dis_y_filtre - lamda2y*(abs(y1-y1_hat)^1/3)*sat(y1 - y1_hat) + y2_dot_des - k1y*(abs(s_y)^1/2)*sat(s_y) - SW2_y);
%  
% %       
...
%       A.U1 = A.m*sqrt(A.Ux^2 + A.Uy^2 + A.Uz^2);
