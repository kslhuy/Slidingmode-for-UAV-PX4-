...
A.X_KP = 50*pi/180;          % KP value in X_position control
A.X_KD = -10*pi/180;         % KD value in X_position control

A.Y_KP = 50*pi/180;          % KP value in Y_position control
A.Y_KD = -10*pi/180;         % KD value in Y_position control

A.phi_KP = 30;      % KP value in roll control
A.phi_KI = 8;     % KI value in roll control            it was .02 !!!!!!!
A.phi_KD = -.8;     % KD value in roll control

% A.phi_KP = .4;   % KP value in roll control
% A.phi_KI = .1; % KI value in roll control
% A.phi_KD = -.07; % KD value in roll control

A.theta_KP = 30;    % KP value in pitch control
A.theta_KI = 8;     % KI value in pitch control
A.theta_KD = -.8;   % KD value in pitch control
...
A.f = 0.025;
A.t = (0:.01:30);
A.alpha=2*pi*A.f*A.t;
% A.t = (0:0.5:3);
% A.alpha=2*pi*A.t;
% A.X_path = [linspace(0,4,4) 2 2 linspace(2,-1.5,7) linspace(-1.5,-1.5,2)];   % the desired path in X
% A.Y_path = [linspace(0,0,4) .5 1 linspace(1.5,1.5,7) linspace(2,2.5,2)];   % the desired path in Y
% % A.Z_path = linspace(0.5,3,length(A.X_path));   % the desired path in Z
% 
A.X_path =  [linspace(0,0,10) , 2*sin(A.alpha)];
A.Y_path =  [linspace(2,2,10) , 2*cos(A.alpha)];

% A.X_path_des_dot = 2*pi*A.f*cos(A.alpha);
% A.Y_path_des_dot = -2*pi*A.f*sin(A.alpha);
% A.X_path_des_ddot = -(2*pi*A.f)^2*sin(A.alpha);
% A.Y_path_des_ddot = -(2*pi*A.f)^2*cos(A.alpha);
% 
% A.Z_path = linspace(0.5,2,length(A.X_path));   % the desired path in Z

% A.X_path = [linspace(0,3,20) linspace(3,3,20) linspace(3,0,20) linspace(0,0,20)];   % the desired path in X
% A.Y_path = [linspace(0,0,20) linspace(0,3,20) linspace(3,3,20) linspace(3,0,20)] ;
A.Z_path = [linspace(2,2,10) linspace(2,2,length(A.X_path)-10)] ;   % the desired path in Z

% A.X_path = [0 0 0 3 3 0 0];   % the desired path in X
% A.Y_path = [0 0 0 0 3 3 0] ;
% A.Z_path = [0.5 1 2 2 2 2 2] ;   % the desired path in Z

% A.X_path_dot = cos(A.var2);
% A.Y_path_dot = cos(A.var2)-1;
% A.Z_path_dot = linspace(0,1.6,length(A.X_path));   % the desired path in Z
...
hold on
plot3(A.X_path,A.Y_path,A.Z_path,'r','linewidth',1.5);
%---------------------------
...
A.Y = 2;        % Initial position in Y direction EF
...
A.Ixx = 3.47E-2;     % Inertia about X axis
A.Iyy = 3.47E-2;     % Inertia about Y axis
A.Izz = 5.31E-2;    % Inertia about Z axis
...
% A.Z_plot = zeros();      % the Z actual value
% A.Z_ref_plot = zeros();  % the Z reference value
% A.Z_dis_plot = zeros();  % the Z disturbance value
% A.Z_kalman_plot = zeros();  % the Z kalman value
% 
% A.X_plot = zeros();      % the X actual value
% A.X_ref_plot = zeros();  % the X reference value
% A.X_dis_plot = zeros();  % the X disturbance value
% A.X_kalman_plot = zeros();  % the X kalman value
% 
% A.Y_plot = zeros();      % the Y actual value
