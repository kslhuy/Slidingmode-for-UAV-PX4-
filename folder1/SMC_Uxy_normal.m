function SMC_Uxy_normal
      ...
      persistent SWx_smoth;
      persistent SWy_smoth;
...
%       SW_y = SW_y + sat(s_y)*A.Ts;
      ...
      SWx_smoth = SWx_smoth + abs(s)^(1/3)*sat(s)*A.Ts;
      SWy_smoth = SWy_smoth + abs(s_y)^(1/3)*sat(s_y)*A.Ts;
...
% % %STSMC
      Ux =  (X_des_ddot + rho*error_dot + K1_x*sqrt(abs(s))*sat(s) + K2_x*SW );
      Uy =  (Y_des_ddot + rho*error_dot_y + K1_y*sqrt(abs(s_y))*sat(s_y) + K2_y*SW_y );
    
% %     
%       Ux = X_des_ddot + rho*error_dot + K1_x*abs(s)^(2/3)*sat(s) + K2_x*SWx_smoth ;
%       Uy =  Y_des_ddot + rho*error_dot_y + K1_y*abs(s_y)^(2/3)*sat(s_y) + K2_y*abs(s_y)^(1/3)*SWy_smoth ;

      A.Uxx(A.counter) = Ux;
      A.Ux_obser(A.counter) = Ux - rho*error_dot - X_des_ddot ;
      
      A.Ux_obser_noSW(A.counter) = Ux - rho*error_dot - X_des_ddot - K1_x*sqrt(abs(s))*sat(s) ;
