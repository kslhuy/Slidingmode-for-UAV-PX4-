...
%         A.y_es(A.counter) = y_0;
      ...
      SWx_smoth = SWx_smoth + abs(s_x)^(1/3)*sat(s_x)*A.Ts;
      SWy_smoth = SWy_smoth + abs(s_y)^(1/3)*sat(s_y)*A.Ts;
  
      A.Ux =  x2_dot_des - cx*A.X_dot  + cx*x1_dot_des - k1x*abs(s_x)^(2/3)*sat(s_x) - k2x*SWx_smoth ;
      A.Uy =  y2_dot_des - cy*A.Y_dot + cy*y1_dot_des - k1y*abs(s_y)^(2/3)*sat(s_y) - k2y*SWy_smoth ;
