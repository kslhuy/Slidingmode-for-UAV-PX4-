'''
Simulation for UAV with Sliding mode controller for outer loop  
'''
import platform
...
for i_global in range(0,plotl-1):
    ...
    phi_ref, theta_ref, U1=support.pos_controller_SMC_3(X_ref[i_global+1],X_dot_ref[i_global+1],X_dot_dot_ref[i_global+1],Y_ref[i_global+1],Y_dot_ref[i_global+1],Y_dot_dot_ref[i_global+1],Z_ref[i_global+1],Z_dot_ref[i_global+1],Z_dot_dot_ref[i_global+1],psi_ref[i_global+1],states)
    ...
    for i in range(0,innerDyn_length):
        ...
        if omega1P2<=0 or omega2P2<=0 or omega3P2<=0 or omega4P2<=0:
            if omega1P2<0 : omega1P2 = -omega1P2
            if omega2P2<0 : omega2P2 = -omega2P2
            if omega3P2<0 : omega3P2 = -omega3P2
            if omega4P2<0 : omega4P2 = -omega4P2
...
copyright=ax0.text(min(X_ref),min(Y_ref)*2,max(Z_ref)*1.8,' ',size=15)
