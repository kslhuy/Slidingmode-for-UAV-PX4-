'''
support file for main file MAIN_LPV_MPC_drone 
Only need to run the main file MAIN_LPV_MPC_drone.py
'''
...
class SupportFilesDrone:
    ...
    def trajectory_generator(self,t):
        ...
        if trajectory==1 or trajectory==2 or trajectory==3 or trajectory==4:
            ...
            print("You only have 10 trajectories. Choose a number from 1 to 10")
            exit()
    ...
    def pos_controller_SMC_4 (self,X_ref,X_dot_ref,X_dot_dot_ref,Y_ref,Y_dot_ref,Y_dot_dot_ref,Z_ref,Z_dot_ref,Z_dot_dot_ref,Psi_ref,states):
        # Get the necessary constants
        Jx=self.constants[0] # kg*m^2
        Jy=self.constants[1] # kg*m^2
        Jz=self.constants[2] # kg*m^2
        ...
        # Sliding surface
        s_x = ex_dot + K_theta*ex
        s_y = ey_dot + K_phi*ey
        s_z = ez_dot + k_z*ez
        
        # SUPER twsing
        v_dot_x = k_2_x*np.sign(s_x)
        v_dot_y = k_2_y*np.sign(s_y)
        v_dot_z = k_2_z*np.sign(s_z)

        self.v_x = self.v_x + v_dot_x*0.04
        self.v_y = self.v_y + v_dot_y*0.04
        self.v_z = self.v_z + v_dot_z
        
        # nonlineaire decoup
        # Compute phi, theta, U1
        
        U1 = m/(np.cos(phi)*np.cos(theta))*(g - Z_dot_dot_ref + k_z*ez_dot + k_1_z*sat(s_z))
        # U1 = m/(np.cos(phi)*np.cos(theta))*(g - Z_dot_dot_ref + k_z*ez_dot + k_1_z*np.abs(s_z)**0.5*np.sign(s_z) + self.v_z )
        U1=U1[0]
        
        x_dd = k_1_x*np.abs(s_x)*np.sign(s_x)
        y_dd = k_1_y*np.abs(s_y)*np.sign(s_y) 
        
        """
        x_dd = k_1_x*sat(s_x) 
        y_dd = k_1_y*sat(s_y)
        """
        x_dd_desire = X_dot_dot_ref + x_dd
        y_dd_desire = Y_dot_dot_ref + y_dd

        Theta_ref = m/U1 * (np.sin(Psi_ref)*y_dd_desire - x_dd_desire*np.cos(Psi_ref))
        Theta_ref = Theta_ref[0] 
        Phi_ref = m/U1 * (x_dd_desire*np.sin(Psi_ref) + y_dd_desire*np.cos(Psi_ref))
        Phi_ref = Phi_ref[0]

        return Phi_ref, Theta_ref, U1
    
    def pos_controller_SMC_3 (self,X_ref,X_dot_ref,X_dot_dot_ref,Y_ref,Y_dot_ref,Y_dot_dot_ref,Z_ref,Z_dot_ref,Z_dot_dot_ref,Psi_ref,states):
        
        # Novel robust super twisting integral sliding mode controller
        # for a quadrotor under external disturbances 2019
        ...
        # Parameters sliding surface PID 
        k_Px = 2
        k_Py = 2
        k_Pz = 2
        ...
        # Parameters sliding surface normal 
        k_x = 1
        k_y = 1
        k_z = 1
        # Parameters u_switch_ST
        k_1_x = k_1_y= k_1yz = 1
        k_2_x = 1
        k_2_y = 1
        k_2_z = 1
        ...
        ## SUPER twsing term of u_swtich 
        v_dot_x = k_2_x*sat(s_x)
        v_dot_y = k_2_y*sat(s_y)
        v_dot_z = k_2_z*sat(s_z)

        self.v_x = self.v_x + v_dot_x*0.4
        self.v_y = self.v_y + v_dot_y*0.4
        self.v_z = self.v_z + v_dot_z*0.4
        
        ##STSMC control
        vx = X_dot_dot_ref + lam_ps*ex_dot + (k_1_x*np.abs(s_x)**0.5*sat(s_x) + self.v_x )
        vy = Y_dot_dot_ref + lam_ps*ey_dot + (k_1_x*np.abs(s_y)**0.5*sat(s_y) + self.v_y )
        vz = Z_dot_dot_ref + lam_ps*ez_dot + (k_1_x*np.abs(s_z)**0.5*sat(s_z) + self.v_z )
        
        # ###SMC_sat control
        # vx = X_dot_dot_ref + (lam_ps*ex_dot + k_1*sat(s_x/0.5))
        # vy = Y_dot_dot_ref + (lam_ps*ey_dot + k_1*sat(s_y/0.5))
        # vz = Z_dot_dot_ref + (lam_ps*ez_dot + k_1*sat(s_z/0.5))
        

        # ## Sliding surface PID

        # s_x = k_Dx*ex_dot + k_Px*ex + k_ix*self.ex_inte
        # s_y = k_Dy*ey_dot + k_Py*ey + k_iy*self.ez_inte
        # s_z = k_Dz*ez_dot + k_Pz*ez + k_iz*self.ez_inte
        # s_x.real
        # s_y.real
        # s_z.real

        # self.ex_inte = self.ex_inte + 0.4*self.ex_inte 
        # self.ey_inte = self.ey_inte + 0.4*self.ey_inte
        # self.ez_inte = self.ez_inte + 0.4*self.ez_inte

        # ### SMPIDC control
        # vx = X_dot_dot_ref + (k_Px*ex_dot + k_ix*ex + k_sx*sat(s_x/0.5))
        # vy = Y_dot_dot_ref + (k_Py*ey_dot + k_iy*ey + k_sy*sat(s_y/0.5))
        # vz = Z_dot_dot_ref + (k_Pz*ez_dot + k_iz*ez + k_sz*sat(s_z/0.5))
        ...
        #Theta_ref= Theta_ref - 0.1*np.sign(Theta_ref)
        if Psi_ref>=0:
    ...
    def pos_controller_SMC_2 (self,X_ref,X_dot_ref,X_dot_dot_ref,Y_ref,Y_dot_ref,Y_dot_dot_ref,Z_ref,Z_dot_ref,Z_dot_dot_ref,Psi_ref,states):
        # Get the necessary constants
        Jx=self.constants[0] # kg*m^2
        Jy=self.constants[1] # kg*m^2
        Jz=self.constants[2] # kg*m^2
        ...
        Theta_ref = m/U1 * (np.cos(Psi_ref)*(-X_dot_dot_ref + K_theta*ex_dot) + np.sin(Psi_ref)*(-Y_dot_dot_ref + K_phi * ey_dot)) + np.cos(Psi_ref)/U1*k_1_x*sat(s_x/1) + np.sin(Psi_ref)/U1*k_1_y*sat(s_y/1)
        Phi_ref   = m/U1 * (np.sin(Psi_ref)*(-X_dot_dot_ref + K_theta*ex_dot) - np.cos(Psi_ref)*(-Y_dot_dot_ref + K_phi * ey_dot)) + np.sin(Psi_ref)/U1*k_1_x*sat(s_x/1) - np.cos(Psi_ref)/U1*k_1_y*sat(s_y/1)
                

        return Phi_ref, Theta_ref, U1

    def pos_controller_SMC (self,X_ref,X_dot_ref,X_dot_dot_ref,Y_ref,Y_dot_ref,Y_dot_dot_ref,Z_ref,Z_dot_ref,Z_dot_dot_ref,Psi_ref,states):
        """ Sliding mode control convention """
        """ with saturation """
        # Get the necessary constants
        Jx=self.constants[0] # kg*m^2
        Jy=self.constants[1] # kg*m^2
        ...
        self.v_z = self.v_z + v_dot_z*0.4
        """
        vx = X_dot_dot_ref + lam_ps*ex_dot + (k_1*np.abs(s_x)**0.5*np.sign(s_x) + self.v_x )
        vy = Y_dot_dot_ref + lam_ps*ey_dot + (k_1*np.abs(s_y)**0.5*np.sign(s_y) + self.v_y )
        vz = Z_dot_dot_ref + lam_ps*ez_dot + (k_1*np.abs(s_z)**0.5*np.sign(s_z) + self.v_z )
        """
        vx = X_dot_dot_ref + (lam_ps*ex_dot + k_1*np.sign(s_x))
        vy = Y_dot_dot_ref + (lam_ps*ey_dot + k_1*np.sign(s_y))
        vz = Z_dot_dot_ref + (lam_ps*ez_dot + k_1*np.sign(s_z))

        # vx = X_dot_dot_ref + (lam_ps*ex_dot + k_1*np.sign(s_x/0.5))
        # vy = Y_dot_dot_ref + (lam_ps*ey_dot + k_1*np.sign(s_y/0.5))
        # vz = Z_dot_dot_ref + (lam_ps*ez_dot + k_1*np.sign(s_z/0.5))
        ...
        U1=U1[0]
        """
        U1 = (vz + g)*m/(np.cos(phi)*np.cos(theta))
        U1=U1[0]
        return Phi_ref, Theta_ref, U1
